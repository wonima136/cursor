...
$filename = $group_name . '_' . $type_name . '_' . $date_for_filename . '.' . $ext;
...
try {
    ...
    foreach ($dates as $date) {
        ...
        // 如果域名数量超过900，分批查询
        if ($allowed_domains !== null && !empty($allowed_domains) && count($allowed_domains) > 900) {
            $batch_size = 900;
            $batches = array_chunk($allowed_domains, $batch_size);
            
            foreach ($batches as $batch) {
                $sql = 'SELECT url, COUNT(*) as count FROM spider_visits WHERE 1=1';
                $params = array();
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if (!isset($url_counts[$url])) {
                        ...
                        $url_domains[$url] = isset($parsed['host']) ? $parsed['host'] : '';
                    }
    ...
    if ($format === 'csv') {
        ...
        foreach ($url_counts as $url => $count) {
            ...
            echo $url_escaped . ',' . $count . ',' . $domain_escaped . "\n";
        }
