...
define('REDIS_SITEMAP_PREFIX', 'sitemap:' . SITE_ID . ':');
...
function saveParasiteTaskToRedis($taskId, $taskData) {
    ...
    try {
        ...
        $redis->hMSet($taskKey, [
            ...
            'updated_at' => $taskData['updated_at'] ?? date('Y-m-d H:i:s'),
            // ⭐ 新增：目标目录功能
            'target_paths_enabled' => !empty($taskData['target_paths_enabled']) ? '1' : '0',
            'target_paths' => $taskData['target_paths'] ?? '',
        ]);
...
function getParasiteTaskFromRedis($taskId) {
    ...
    try {
        ...
        // ⭐ 新增：目标目录功能
        $task['target_paths_enabled'] = ($task['target_paths_enabled'] ?? '0') === '1';
...
function deleteParasiteTaskFromRedis($taskId) {
    ...
    try {
        ...
        $keys = [
            ...
            "{$prefix}task:{$taskId}:spider_filter", // ⭐ 新增：删除蜘蛛筛选器
        ];
        ...
        // 删除目录级别的统计数据
        $dirKeys = $redis->keys("{$prefix}task:{$taskId}:dir:*");
        if (!empty($dirKeys)) {
 ...
 * 保存地图重定向任务配置到 Redis
 */
function saveSitemapTaskToRedis($taskId, $taskData) {
    $redis = getRedis();
    if (!$redis) return false;
    ...
    try {
        ...
        $configKey = "{$prefix}task:{$taskId}:config";
        $redis->set($configKey, json_encode($taskData));
        return true;
    } catch (Exception $e) {
        error_log("保存地图重定向任务到Redis失败: " . $e->getMessage());
        return false;
    }
}
 ...
 * 从 Redis 获取地图重定向任务配置
 */
function getSitemapTaskFromRedis($taskId) {
    $redis = getRedis();
    if (!$redis) return null;
    ...
    try {
        ...
        error_log("从Redis获取地图重定向任务失败: " . $e->getMessage());
        return null;
    }
}

/**
 * 从 Redis 删除地图重定向任务（包括所有相关数据）
 */
function deleteSitemapTaskFromRedis($taskId) {
    $redis = getRedis();
    if (!$redis) return false;
    ...
    try {
        ...
        // 删除任务配置
        $redis->del("{$prefix}task:{$taskId}:config");
        
        // 删除任务统计
        $redis->del("{$prefix}task:{$taskId}:stats");
        
        // 删除内页统计
        $redis->del("{$prefix}task:{$taskId}:inner:stats");
        
        // 删除预抓取状态
        $redis->del("{$prefix}task:{$taskId}:prefetch_status");
        
        // 删除所有域名的缓存和计数
        // 获取所有相关的 keys
        $domainKeys = $redis->keys("{$prefix}task:{$taskId}:domain:*");
        if (!empty($domainKeys)) {
        ...
        error_log("从Redis删除地图重定向任务失败: " . $e->getMessage());
        return false;
    }
}
 ...
 * 更新地图重定向任务统计
 */
function incrementSitemapTaskStats($taskId, $field = 'total', $increment = 1) {
    $redis = getRedis();
    if (!$redis) return false;
    ...
    try {
        ...
        error_log("更新地图重定向统计失败: " . $e->getMessage());
        return false;
    }
}
 ...
 * 获取地图重定向任务统计
 */
function getSitemapTaskStats($taskId) {
    $redis = getRedis();
    if (!$redis) return [];
    ...
    try {
        ...
        error_log("获取地图重定向统计失败: " . $e->getMessage());
        return [];
    }
}

/**
 * 更新地图重定向内页跳转统计（用于TOP排行）
 */
function incrementSitemapInnerPageStats($taskId, $url) {
    $redis = getRedis();
    if (!$redis) return false;
    
    try {
        $prefix = REDIS_SITEMAP_PREFIX;
        $innerStatsKey = "{$prefix}task:{$taskId}:inner:stats";
        $redis->zIncrBy($innerStatsKey, 1, $url);
        return true;
    } catch (Exception $e) {
        error_log("更新地图重定向内页统计失败: " . $e->getMessage());
        return false;
    }
}
 ...
 * 获取地图重定向内页跳转TOP排行
 */
function getSitemapInnerPageTop($taskId, $limit = 10) {
    $redis = getRedis();
    if (!$redis) return [];
    ...
    try {
        ...
        $innerStatsKey = "{$prefix}task:{$taskId}:inner:stats";
        // 获取分数最高的前N个（降序）
        $top = $redis->zRevRange($innerStatsKey, 0, $limit - 1, true);
        return $top ?: [];
    } catch (Exception $e) {
        error_log("获取地图重定向内页TOP失败: " . $e->getMessage());
        return [];
    }
}
 ...
 * 清除地图重定向域名缓存
 */
function clearSitemapDomainCache($taskId, $domain) {
    $redis = getRedis();
    if (!$redis) return false;
    ...
    try {
        ...
        error_log("清除地图重定向域名缓存失败: " . $e->getMessage());
        return false;
    }
}
