    ...
    private function getEnabledTasks() {
        ...
        foreach ($taskKeys as $key) {
            ...
            $tasks[] = [
                ...
                'probability' => intval($taskConfig['probability'] ?? $taskConfig['settings']['probability'] ?? 100), // ⭐ 新增：概率控制
        ...
        foreach ($rules as $rule) {
            ...
            if ($matched) {
                ...
                if ($probability < 100 && mt_rand(1, 100) > $probability) {
                    $this->logger->debug("BigsiteRedirect: 任务 {$task['name']} 概率未命中 ({$probability}%)，不跳转");
                    return null; // 概率未命中，不跳转
                }
