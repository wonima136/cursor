...
function _r301parasite_update($taskId, $data) {
    ...
    foreach ($tasks as &$task) {
        if ($task['id'] === $taskId) {
            ...
            // ⭐ 新增：目标目录功能
            if (isset($data['target_paths_enabled'])) {
                $task['target_paths_enabled'] = (bool)$data['target_paths_enabled'];
            }
            if (isset($data['target_paths'])) {
                $task['target_paths'] = trim($data['target_paths']);
            }
