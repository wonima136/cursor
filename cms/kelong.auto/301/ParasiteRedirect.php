...
class ParasiteRedirect extends RedirectModule {
    ...
    private function checkByDirectory($task) {
        ...
        // 互连模式：如果没有配置目标域名，从源域名中选择（排除当前域名）
        if ($redirectMode === 'interlink' && empty($targetDomains) && !empty($sourceDomains)) {
            $availableDomains = [];
            ...
            $targetDomains = $availableDomains;
        }
        
        if (!empty($sourcePath) && !empty($targetDomains)) {
            // 检查当前域名是否在源域名列表中
            $domainMatched = false;
            ...
            // 检查当前URI是否在源目录下
            if ($domainMatched && $this->matchDirectory($sourcePath)) {
                // 检查概率
                ...
                if ($pathMode === 'strip_prefix') {
                    ...
                    $uri = substr($this->currentUri, strlen($normalizedPath) - 1);
                    if (empty($uri) || $uri === '/') {
                        $uri = '/';
                    }
                } else {
                ...
                // 应用URI替换规则
                $uri = $this->applyUriReplacements($task, $uri);
                
                $targetUrl = $protocol . '://' . $targetDomain . $uri;
