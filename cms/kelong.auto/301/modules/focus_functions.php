...
function _focus_getDB() {
    ...
    if ($db === null) {
        ...
        error_log("数据库文件存在: " . (file_exists($dbFile) ? '是' : '否'));
        
        if (!file_exists($dbFile)) {
            throw new Exception('Focus数据库不存在，请先运行初始化脚本: ' . $dbFile);
        }
 ...
 * 提取链接
 */
function _focus_extractLinks($taskId, $sourceType, $sourceValue, $keywords = '', $matchFromData = true, $matchFromInput = true) {
    try {
        $db = _focus_getDB();
        ...
        if (!empty($links)) {
            ...
            foreach ($links as $link) {
                ...
                $insertStmt->bindValue(2, $link['full_url'], SQLITE3_TEXT);
                $insertStmt->bindValue(3, $link['keyword'] ?? '', SQLITE3_TEXT);
                $insertStmt->bindValue(4, $link['redirect_count'] ?? 0, SQLITE3_INTEGER);
                $insertStmt->execute();
                $insertStmt->reset();
            }
        }
...
function _focus_saveTaskConfig($taskId, $config) {
    try {
        ...
        $stmt->bindValue(7, $taskId, SQLITE3_TEXT);
        $stmt->execute();
        ...
        return ['success' => true, 'message' => '保存成功', 'locked_count' => $lockResult['locked_count'] ?? 0];
...
function _focus_lockExtractedLinks($db, $taskId, $targetUrl, $scheduleInterval) {
    try {
        ...
        return ['success' => true, 'locked_count' => $successCount];
