    ...
    <form id="updateForm">
      <div class="form-card mb-3">
        ...
        <div class="field-block">
          <div class="d-flex align-items-center gap-3 mb-0" id="tagToggleRow">
            <input type="checkbox" class="form-check-input field-check" id="chk_tags"
                   onchange="toggleField('tags_row', this)">
            <label class="form-label mb-0 toggle-label" for="chk_tags">
              <i class="bi bi-tags me-1 text-primary"></i>标签
            </label>
          </div>
          <div id="tags_row" class="field-row mt-3 flex-column gap-2" style="display:none">
            <div class="d-flex gap-2">
              <select name="tag_mode" class="form-select form-select-sm" style="width:130px">
                <option value="add">添加标签</option>
                <option value="replace">替换全部标签</option>
                <option value="remove">移除标签</option>
                <option value="clear">清除所有标签</option>
              </select>
              <span class="small text-muted align-self-center">选择操作方式</span>
            </div>
            <div class="d-flex flex-wrap gap-2" id="tagPickerArea">
              <?php foreach ($allTags as $tag): ?>
              <label class="d-flex align-items-center gap-1" style="cursor:pointer">
                <input type="checkbox" name="tag_ids[]" value="<?= $tag['id'] ?>" class="form-check-input">
                <span class="tag-pill" style="background:<?= h($tag['color']) ?>"><?= h($tag['name']) ?></span>
              </label>
              <?php endforeach; ?>
              <?php if (!$allTags): ?><span class="text-muted small">暂无标签，请先创建</span><?php endif; ?>
            </div>
          </div>
        </div>
          ...
          <div class="d-flex align-items-center gap-3">
            ...
            <div id="dns_row" class="field-row flex-grow-1" style="display:none">
              <input type="text" name="dns_servers" class="form-control form-control-sm"
                     placeholder="ns1.example.com,ns2.example.com（逗号分隔）">
            </div>
          </div>
        </div>
...
function submitUpdate() {
  ...
  checked.forEach(function(chk) {
    ...
    switch (fieldId) {
      case 'tags':
        var mode    = data.get('tag_mode') || 'add';
        var tagIds  = data.getAll('tag_ids[]').map(Number);
        payload.fields.tags = { mode: mode, ids: tagIds };
        break;
      case 'status':
      ...
      case 'icp':
        ...
        break;
      case 'registrar':
        payload.fields.registrar_id = parseInt(data.get('registrar_id')) || 0;
        break;
      case 'account':
        payload.fields.account_id = parseInt(data.get('account_id')) || 0;
        break;
      case 'group':
  ...
  fetch('/api/batch_update_job.php', {
    method: 'POST',
