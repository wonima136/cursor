...
// ── 缓存文件路径 ──────────────────────────────────────────────
$cacheDir  = dirname(__DIR__) . '/data/agg';
if (!is_dir($cacheDir)) mkdir($cacheDir, 0755, true);
$cacheKey  = md5(serialize(['f' => $filters, 't' => $tagFilter]));
$cacheFile = $cacheDir . '/agg_' . $cacheKey . '.json';

// GET 请求：有日期筛选时跳过缓存（实时计算）；否则从缓存返回
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    $hasDateFilter = !empty($filters['date_from']) || !empty($filters['date_to']);
    if (!$hasDateFilter && file_exists($cacheFile)) {
        readfile($cacheFile);
    } else {
        // 无缓存或有日期筛选 → 计算新鲜数据（不写缓存，日期组合无限多）
        if ($hasDateFilter) {
            try {
                $groups  = getFilterAggregates($filters, $tagFilter);
